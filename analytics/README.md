# Analytics Module

This module contains Titanic dataset analysis and basic data cleaning.

## Files
- `01_eda.py`: EDA and cleaning script for the Titanic dataset
- `02.modeling.py`: Titanic classification and regression modeling script
- `titanic.csv`: local Titanic dataset copy used by the analysis
- `titanic_cleaned.csv`: cleaned version of the Titanic dataset (generated by the script)

## What it does
- Loads `analytics/titanic.csv`
- Prints dataset shape, data info, and summary statistics
- Computes missing-value percentages for each column
- Handles missing values for `age`, `embarked`, `embark_town`, and `deck`
- Builds classification and regression pipelines with model comparison, SMOTE, and pipeline persistence
- Optionally writes cleaned data to `analytics/titanic_cleaned.csv`

## Run

From the project root:

```bash
python analytics/01_eda.py
```

```bash
python analytics/02.modeling.py
```

## Requirements

Install the project dependencies from the root:

```bash
pip install -r requirements.txt
```

## Notes

- The analytics module is self-contained and uses the local Titanic dataset in `analytics/titanic.csv`.
- If you add new analytics scripts, place them in the `analytics/` folder and document them here.
